...
(function () {
  ...
  /* ---------------- scroll mapping and the frame loop ---------------- */
  var LV = null;
  function resize() {
  ...
  function frame(time, dt) {
    ...
    var qT = NK.clamp(-r.top / span, 0, 1);
    if (time - lastT > 0.5 || Math.abs(qT - qCur) > 0.12) qCur = qT;
    ...
    if (ph.b !== undefined) {
      ...
      var asp = W / H;
      /* the lens must reach the farthest screen corner exactly at p = 1 */
      var far = 0;
    ...
    stage.style.setProperty('--qo', NK.smooth(0.9, 0.97, qCur).toFixed(3));
  }
