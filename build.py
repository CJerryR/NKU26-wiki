...
# Homepage chrome partials are injected by the template, never as sections.
...
def expand_home_partials(body):
    ...
    def include(match):
        name = match.group(1)
        path = PARTIALS / "home" / f"{name}.html"
        if name in HOME_CHROME or not path.is_file():
...
{body}
    ...
    home_scripts = ""
    if is_home:
        home_styles = "\n  ".join(f'<link rel="stylesheet" href="{P}css/{name}.css" />' for name in HOME_STYLES)
        payload = json.dumps(home_data(), ensure_ascii=False).replace("<", "\\u003c")
        home_scripts = '<script>window.NKU_HOME = ' + payload + ';</script>\n  '
        home_scripts += "\n  ".join(f'<script src="{P}js/{name}.js" defer></script>' for name in HOME_SCRIPTS)
