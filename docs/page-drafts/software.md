...
heading: The code
sub: The source-built static publishing system behind this wiki, generated by GitLab CI/CD into iGEM's public artifact directory.
meta: Stack=HTML + CSS + JavaScript | Build=Python 3 | Delivery=GitLab CI/CD
---
...
## Supporting infrastructure, not a Software prize claim

This page documents the publishing infrastructure used to build the team wiki. It is not submitted as the separate 2026 Software deliverable and does not, by itself, qualify the team for Best Software. A competition wiki is a multi-page publication that must stay consistent while it is edited, remain reproducible from source, and avoid non-iGEM runtime dependencies.
...
Where the result lands is decided by metadata too. `route: software` writes this page to `public/software/index.html`; a page with no route falls back to `public/pages/<name>.html`; and `layout: home`, or the file name `index`, writes `public/index.html`. The relative asset prefix follows from the depth of that output path, so the generated site remains portable on iGEM's host.

**Fig 1** One build: editable sources in, a disposable `public/` artifact out. GitLab CI/CD publishes the artifact; generated pages are not committed as source.
...
The disposable `public/` directory is recreated before anything new is written, so renamed or retired routes cannot linger as stale pages.
...
The route metadata decides the path inside `public/`. Nested routes are written as `public/route/index.html`, and the relative asset prefix is calculated from how deep that file sits.
...
The local command is `python3 build.py`; GitLab CI/CD runs `python3 build.py --output public`. Python 3 is the only build dependency, so the output is deterministic without downloading packages during the pipeline.
...
Use the visible **Source repository** link in the footer to inspect the build script, templates, page sources, design system, and interaction layer. On the official iGEM deployment this link is injected from GitLab's `CI_PROJECT_URL`.
